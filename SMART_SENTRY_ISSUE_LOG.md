...
- **Notes**: For any torch subpackage whose `__init__` imports a C extension, the correct bundling workaround is: direct `glob.glob` scan of `torch/<subpkg>/**/*.py` → build `datas` tuples as `(src_path, dest_dir)`. Related: ISS-081 (hiddenimports — ineffective), ISS-082 v1 (collect_data_files — ineffective, returned empty).
