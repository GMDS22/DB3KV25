...
|---|---|---|
| `Ctrl+Alt+Q` | Open Quick Keys reference | Same action as the top-row `Quick Keys` button |
| `Ctrl+Alt+E` | Enable or disable Smart Sentry | Toggles the main sentry runtime |
| `Ctrl+Alt+C` | Toggle connection | Connects or disconnects the current transport |
| `Ctrl+Alt+O` | Open or close camera | Uses the current camera source setting |
| `Ctrl+Alt+V` | Show or hide video | Detection can continue while preview is hidden |
| `Ctrl+Alt+W` | Wake Up / go home | Uses the guided home movement path |
| `Ctrl+Alt+R` | Go Rest | Uses the guided rest movement path |
