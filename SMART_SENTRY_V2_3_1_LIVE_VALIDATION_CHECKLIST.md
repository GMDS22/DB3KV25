# Smart Sentry v2.3.1 Live Validation Checklist

Use this after packaging the desktop app and before treating the v2.3.1 build as release-ready.

## 1. Launch And Identity

1. Launch the packaged app from the v2.3.1 release folder on `F:`.
2. Confirm the window title reads `SMART SENTRY V2.3.1`.
...
7. Confirm the turret returns to the configured guard position after the recovery sequence completes.
8. Repeat once more to ensure the behavior is stable across two consecutive target-loss events.
...
4. Confirm manual movement still works even after a target-loss recovery cycle.
...
1. Release identity matches `2.3.1` in the window title and version markers.
2. Connection and status surfaces are readable under live tracking.
3. Scope view matches the approved monochrome contract.
4. Fixed-guard target loss returns to guard cleanly.
