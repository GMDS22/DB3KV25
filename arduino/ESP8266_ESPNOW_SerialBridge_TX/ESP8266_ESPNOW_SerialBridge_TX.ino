// ESP8266 ESPNOW Serial Bridge (TX / turret-side)
// UART Serial <-> ESP-NOW
//
// Usage is symmetric with the RX sketch.
// - Flash this to the ESP8266 that lives on the turret.
// - Wire its UART (3.3V TTL) to your turret controller UART.
// - Set PEER_MAC to the USB-side ESP8266 MAC for best reliability.
...
void setup() {
  ...
  Serial.println();
  Serial.println("ESP8266 ESPNOW SerialBridge TX (turret-side)");
  Serial.print("My MAC: ");
  uint8_t mac[6];
  WiFi.macAddress(mac);
  printMac(mac);
