...
static const int SERVO_FIRE_DEG = 45;
...
static int trigger_servo_rest_deg = SERVO_REST_DEG;
static int trigger_servo_fire_deg = SERVO_FIRE_DEG;
static int trigger_servo_speed_dps = SERVO_SPEED_DPS_DEFAULT;
static float trigger_servo_current_deg = (float)SERVO_REST_DEG;
static float trigger_servo_target_deg = (float)SERVO_REST_DEG;
...
static int clampServoAngle(int deg) {
  return clampInt(deg, 0, 180);
}

static void triggerServoWriteDeg(int deg) {
  ledcWrite(PIN_TRIGGER_SERVO, degToDutyTicks(clampServoAngle(deg)));
}

static void setTriggerServoTarget(bool on) {
  trigger_servo_target_deg = (float)(on ? clampServoAngle(trigger_servo_fire_deg)
...
static void updateTriggerServoMotion() {
  ...
  float max_step = (float)max(10, trigger_servo_speed_dps) * ((float)elapsed_ms / 1000.0f);
