...
$sketchDir = Join-Path $repoRoot "arduino\SMART_SENTRY_V3_0_WAVESHARE_UDP_BUS_BRIDGE"
$buildDir = Join-Path $repoRoot "logs\precision_tuning\waveshare_bridge_build_manual"
...
Write-Host "[MANUAL-FLASH] Compiling patched Waveshare bridge"
& $cliPath compile --fqbn $Fqbn --build-path $buildDir $sketchDir
if ($LASTEXITCODE -ne 0) {
    throw "Compile failed"
}

$bootloader = Join-Path $buildDir "SMART_SENTRY_V3_0_WAVESHARE_UDP_BUS_BRIDGE.ino.bootloader.bin"
$partitions = Join-Path $buildDir "SMART_SENTRY_V3_0_WAVESHARE_UDP_BUS_BRIDGE.ino.partitions.bin"
$appBin = Join-Path $buildDir "SMART_SENTRY_V3_0_WAVESHARE_UDP_BUS_BRIDGE.ino.bin"
...
Write-Host "[MANUAL-FLASH] Flashing patched bridge to $Port"
